    ...
    <div class="wrapper">
      <ul class="top">
        ...
        <li class="right">
          <el-button
            type="primary"
            @click="toAnswer(examData)"
          >开始学习</el-button>
        </li>
        <li v-if="examData.recordid != ''&&examData.recordid != null" class="right">
          <el-button
            type="sucess"
            @click="toAnswers(examData)"
          >测试结果</el-button>
        </li>
      </ul>
      <!--     <ul class="bottom">
...
export default {
  ...
  methods: {
    ...
    toAnswer(exam) {
      ...
      this.$router.push({
        path: '/trainAnswer',
        query: { exam: exam, isPractice: false, recordid: exam.recordid }
      })
    }
  }
}
