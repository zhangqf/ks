  ...
  <div id="answer">
    ...
    <div class="top">
      <ul class="item">
        ...
        <li>欢迎{{ currentUser }}参加 {{ examData.name }} 课程</li>
        <!--         <li @click="flag = !flag">
        ...
        <li style="margin-left: 20px">
          <i class="iconfont icon-menufold icon20" />
        </li>
      </ul>
    </div>
      ...
      <div class="flexarea ui-right">
        ...
        <transition name="slider-fade">
          <div class="right">
            <div class="title">
              <p>{{ title }}</p>
              <i class="iconfont icon-right auto-right" />
              ...
              <template v-else>
                <span>全卷共{{ topicCount[0] + topicCount[1] + topicCount[2] }}题
                  <i class="iconfont icon-time" />倒计时：<b>{{ time }}</b>分钟
                </span>
              </template>
            </div>
            <div class="content">
              ...
              <template v-if="this.topic[0].length != 0">
                <div v-if="currentType == 0" class="fill">
                  ...
                  <div v-if="isPractice" class="analysis">
                    <ul>
                      <li v-if="fillList[index]">
                        <el-tag type="success">你的答案：</el-tag><span class="right">{{
                          fillList[index] == "?" ? "未作答" : fillList[index]
                        }}</span>
                      </li>
                      <li>
                        <el-tag type="success">正确答案：</el-tag><span class="right">{{
                          topic[0][index].questionAnswer
                        }}</span>
                      </li>
                      <li><el-tag>题目解析：</el-tag></li>
                      <li>
                        {{
                          topic[0][index].questionExplain == null
                            ? "暂无解析"
                            : topic[0][index].questionExplain
                        }}
                      </li>
                    </ul>
                  </div>
              ...
              <template v-if="this.topic[1].length != 0">
                <div v-if="currentType == 1" class="judge">
                  <el-radio-group
                  ...
                  <div v-if="isPractice" class="analysis">
                    <ul>
                      <li v-if="judgeList[index]">
                        <el-tag type="success">你的答案：</el-tag><span class="right">{{
                          judgeList[index] == "?" ? "未作答" : judgeList[index]
                        }}</span>
                      </li>
                      <li>
                        <el-tag type="success">正确答案：</el-tag><span class="right">{{
                          topic[1][index].questionAnswer
                        }}</span>
                      </li>
                      <li><el-tag>题目解析：</el-tag></li>
                      <li>
                        {{
                          topic[1][index].questionExplain == null
                            ? "暂无解析"
                            : topic[1][index].questionExplain
                        }}
                      </li>
                    </ul>
                  </div>
                ...
                <div v-if="currentType == 2">
                  ...
                  >
                    <el-radio
                      :label="answerA"
                      :disabled="isDisable"
                    >A. {{ showAnswer.choiceA }}</el-radio>
                    <el-radio
                      :label="answerB"
                      :disabled="isDisable"
                    >B. {{ showAnswer.choiceB }}</el-radio>
                    <el-radio
                      :label="answerC"
                      :disabled="isDisable"
                    >C. {{ showAnswer.choiceC }}</el-radio>
                    <el-radio
                      :label="answerD"
                      :disabled="isDisable"
                    >D. {{ showAnswer.choiceD }}</el-radio>
                  </el-radio-group>
                  <div v-if="isPractice" class="analysis">
                    <ul>
                      <li v-if="singleList[index]">
                        <el-tag type="success">你的答案：</el-tag><span class="right">{{
                          singleList[index] == "?"
                            ? "未作答"
                            : singleList[index]
                      ...
                      <li>
                        <el-tag type="success">正确答案：</el-tag><span class="right">{{
                          topic[2][index].questionAnswer
                        }}</span>
                      </li>
                      <li><el-tag>题目解析：</el-tag></li>
                      <li>
                        {{
                          topic[2][index].questionExplain == null
                            ? "暂无解析"
                            : topic[2][index].questionExplain
                        }}
                      </li>
                    </ul>
                  </div>
                ...
                <div v-if="currentType == 3">
                  ...
                  >
                    <el-checkbox
                      :label="answerA"
                      :disabled="isDisable"
                    >A. {{ showAnswer.choiceA }}</el-checkbox>
                    <el-checkbox
                      :label="answerB"
                      :disabled="isDisable"
                    >B. {{ showAnswer.choiceB }}</el-checkbox>
                    <el-checkbox
                      :label="answerC"
                      :disabled="isDisable"
                    >C. {{ showAnswer.choiceC }}</el-checkbox>
                    <el-checkbox
                      :label="answerD"
                      :disabled="isDisable"
                    >D. {{ showAnswer.choiceD }}</el-checkbox>
                  </el-checkbox-group>
                  <div v-if="isPractice" class="analysis">
                    <ul>
                      <li v-if="multiList[index]">
                        <el-tag type="success">你的答案：</el-tag><span class="right">{{
                          multiList[index] == "?" ? "未作答" : multiList[index]
                        }}</span>
                      </li>
                      <li>
                        <el-tag type="success">正确答案：</el-tag><span class="right">{{
                          topic[3][index].questionAnswer
                        }}</span>
                      </li>
                      <li><el-tag>题目解析：</el-tag></li>
                      <li>
                        {{
                          topic[3][index].questionExplain == null
                            ? "暂无解析"
                            : topic[3][index].questionExplain
                        }}
                      </li>
                    </ul>
                  </div>
        ...
        <!--右边选择答题区-->
        <transition name="slider-fade">
          <div v-if="slider_flag" class="left">
            <ul class="l-top">
              <li>
                <a href="javascript:;" />
                <span>当前</span>
            ...
            <div class="l-bottom">
                  ...
                  <ul>
                    <li v-for="(item, index1) in topic[0]" :key="index1">
                      <a
                        href="javascript:;"
                        :class="{
                          border: index == index1 && currentType == 0,
                          bg: fillAnswer[index1][3] != 0
                        }"
                        @click="fill(index1)"
                      ><span
                        :class="{ mark: topic[0][index1].isMark == true }"
                      />{{ index1 + 1 }}</a>
                    </li>
                  </ul>
                </div>
                  ...
                  <ul>
                    <li v-for="(list, index2) in topic[1]" :key="index2">
                      <a
                        href="javascript:;"
                        :class="{
                          border: index == index2 && currentType == 1,
                          bg: bg_flag && topic[1][index2].isClick == true
                        }"
                        @click="judge(index2)"
                      ><span
                        :class="{ mark: topic[1][index2].isMark == true }"
                  ...
                  <ul>
                    <li v-for="(list, index3) in topic[2]" :key="index3">
                      <a
                        href="javascript:;"
                        :class="{
                          border: index == index3 && currentType == 2,
                          bg: bg_flag && topic[2][index3].isClick == true
                        }"
                        @click="change(index3)"
                      >
                        <span
                          :class="{ mark: topic[2][index3].isMark == true }"
                        />
                  ...
                  <ul>
                    <li v-for="(list, index4) in topic[3]" :key="index4">
                      <a
                        href="javascript:;"
                        :class="{
                          border: index == index4 && currentType == 3,
                          bg: bg_flag && topic[3][index4].isClick == true
                        }"
                        @click="multiChange(index4)"
                      >
                        <span
                          :class="{ mark: topic[3][index4].isMark == true }"
                        />
                        {{
                          topicCount[0] +
                            topicCount[1] +
              ...
              <div v-if="isPractice" class="final" @click="close()">关闭</div>
              <div v-else class="final" @click="commit()">结束测试</div>
            </div>
          </div>
        </transition>
      </div>
...
export default {
        ...
        sources: [
          {
            ...
            // src: require("D:\\java\\vue\\boot-vue-online-exam-client-master\\src\\assets\\video.mp4") //url地址
          }
        ],
        // poster: "D:\\java\\vue\\boot-vue-online-exam-client-master\\src\\assets\\video.mp4", //你的封面地址
  ...
  created() {
    ...
    console.log(this.$route.query.exam.id)
    console.log(this.isPractice)
    if (this.$route.query.exam.id == undefined) {
  ...
  methods: {
    ...
    playerStateChanged(e) {
      ...
      //   } else {
      //     if (
      //       e.timeupdate >= this.videoStopTime &&
      //       e.timeupdate <= this.videoStopTime + 0.2
      //     ) {
    ...
    getExamData() {
      ...
      _this.videoStopTime = _this.examData.videotime
      _this.playerOptions.sources[0].src =
        process.env.VUE_APP_BASE_API + '' + _this.examData.videopath
      console.log(_this.examData)
    },
    getViewData() {
      const recordId = this.$route.query.recordid // 考试记录id
      const params = {
        recordId: recordId
      }
      // 查询试卷的详细信息
      viewExamRecord(params).then(response => {
        ...
        if (response.data.code === 200) {
          ...
          console.log(response.data.data.exam.videopath)
          ...
          this.playerOptions.sources[0].src =
            process.env.VUE_APP_BASE_API + '' + this.examData.videopath
          // 问题详情
          const dataMap = response.data.data.questionMap

          this.topic = dataMap

          const reduceAnswer = this.topic[2][this.index]
          ...
          for (let i = 0; i < this.topic[3].length; i++) {
            //
            if (this.multiList[i] == '?' && this.multiList[i] != undefined) {
              // 没有答题
              this.multi[i] = []
            } else {
              // 填充答题记录
          ...
          const len = this.topicCount[0]
          const father = []
          for (let i = 0; i < len; i++) {
            // 根据填空题数量创建二维空数组存放每道题答案
            const children = [null, null, null, 0]
            console.log(this.fillList[i])
            if (this.fillList[i] != '?' && this.fillList[i] != undefined) {
              //
              const fillArr = this.fillList[i].split(',')
          ...
          console.log(this.topicCount[1])
          for (let i = 0; i < this.topicCount[1]; i++) {
            this.judgeAnswer = this.judgeList
            console.log(this.judgeList)
            if (this.judgeList[i] != '?' && this.judgeList[i] != undefined) {
          ...
          for (let i = 0; i < this.topicCount[2]; i++) {
            if (this.singleList[i] != '?' && this.singleList[i] != undefined) {
    ...
    getTopicData() {
      const exam = this.$route.query.exam // 试卷
    ...
    fillBG() {
      // 填空题已答题目 如果已答该题目,设置第四个元素为需要填空的个数

      // 只要填了一个空，就是答题了，没有作答，就是null或者空串
      if (
        this.fillAnswer[this.index][0] ||
        this.fillAnswer[this.index][1] ||
        this.fillAnswer[this.index][2]
      ) {
        console.log(121)
    ...
    fill(index) {
      ...
      const Data = this.topic[0]
      this.showQuestion = Data[index].questionContent // 获取题目信息
      if (this.showQuestion.split('()').length == 1) {
        this.part = 1
      } else {
        const part = this.showQuestion.split('()').length - 1 // 根据题目中括号的数量确定填空横线数量
        this.part = part
      }
      this.number = index + 1
    },
    ...
    previous() {
      ...
      if (this.number >= 1 && this.number <= this.topicCount[0]) {
        ...
        this.number > this.topicCount[0] &&
        this.number <= this.topicCount[0] + this.topicCount[1]
      ) {
        // 如果是单选题的第一题，点了上一题
        ...
        this.number > this.topicCount[0] + this.topicCount[1] &&
        this.number <=
          this.topicCount[0] + this.topicCount[1] + this.topicCount[2]
      ) {
        // 如果是多选题的第一题，点了上一题
    ...
    commit() {
      console.log(1223123)
      if (this.time != 0) {
        this.$confirm('课程结束时间未到,是否提前完成', '友情提示', {
          ...
          .then(() => {
            ...
            // 相差的分钟数
            const joinTimeCost = Math.round(ms / 1000.0 / 60)

            // 最终得分
            let finalScore = 0
            /** 计算填空题总分 */
            const fillAnswer = this.fillAnswer // 填空题答案
            let joinAnswerFill = '' // 拼接填空题答案，用于提交给后台记录
            fillAnswer.forEach((element, index) => {
              // 解析答案，用数组存储
              const answerArr = this.topic[0][index].questionAnswer.split(',')
              // 如果答了题，element[3]不为0
              if (element[3] != 0) {
                // 如果3个空的填空题
                if (element[3] == 3) {
                  let cnt = 0 // 正确的个数
                  if (element[0] == answerArr[0]) {
                    // 判断填空答案是否与数据库一致
                    // console.log("第一空正确")
                    cnt++
                  }
                  if (element[1] == answerArr[1]) {
                    // 判断填空答案是否与数据库一致
                    // console.log("第二空正确")
                    cnt++
                  }
                  if (element[2] == answerArr[2]) {
                    // 判断填空答案是否与数据库一致
                    // console.log("第三空正确")
                    cnt++
                  }
            ...
            // 去掉最后一个星号
            joinAnswerFill =
              joinAnswerFill.substring(joinAnswerFill.length - 1) == '*'
                ? joinAnswerFill.substring(0, joinAnswerFill.length - 1)
                : joinAnswerFill

            /** 计算判断题总分 */
            const topic3Answer = this.judgeAnswer
            let joinAnswerJudge = ''
            topic3Answer.forEach((element, index) => {
              ...
              if (right == this.topic[1][index].questionAnswer) {
                // 当前选项与正确答案对比
            ...
            // 去掉最后一个星号
            joinAnswerJudge =
              joinAnswerJudge.substring(joinAnswerJudge.length - 1) == '*'
                ? joinAnswerJudge.substring(0, joinAnswerJudge.length - 1)
                : joinAnswerJudge

            /* 计算单选题总分 */
            const topic1Answer = this.topic1Answer
            let singleAnswer = ''
            topic1Answer.forEach((element, index) => {
              ...
              if (right == this.topic[2][index].questionAnswer) {
                // 当前选项与正确答案对比
            ...
            // 去掉最后一个星号
            singleAnswer =
              singleAnswer.substring(singleAnswer.length - 1) == '*'
                ? singleAnswer.substring(0, singleAnswer.length - 1)
                : singleAnswer

            /* 计算多选题总分 */
            const multiAnswer = this.multi
            let joinAnswerMulti = ''
            multiAnswer.forEach((element, index) => {
              // 循环每道选择题根据选项计算分数
              let right = ''
              element = '' + element
              if (
                ...
                // 去掉最后一个逗号
                right =
                  right.substring(right.length - 1) == ','
              ...
              if (right == this.topic[3][index].questionAnswer) {
                // 当前选项与正确答案对比
            ...
            // 去掉最后一个星号
            joinAnswerMulti =
              joinAnswerMulti.substring(joinAnswerMulti.length - 1) == '*'
                ? joinAnswerMulti.substring(0, joinAnswerMulti.length - 1)
                : joinAnswerMulti

            // 计算考试分数等级
            let joinLevel = 1
            const totalScore = this.examData.totalScore * 1.0
            ...
            // 提交考试记录到后台
            const data = {
              trainId: this.examData.id, // 考试安排ID
              userId: getUserId(),
              finishTime: Date.now(),
