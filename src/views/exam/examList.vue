  ...
  <div id="myExam">
    ...
    <div class="wrapper">
      <ul class="top">
        ...
        <li class="search-li"><div class="icon"><input v-model="filters.keyword" type="text" placeholder="试卷名称" class="search"><i class="el-icon-search" /></div></li>
        <li><el-button type="primary" @click="search()">搜索试卷</el-button></li>
      </ul>
      <ul v-loading="listLoading" class="paper">
        <li v-for="(item,index) in tableData" :key="index" class="item" @click="toExamMsg(item)">
          <!-- <h4 @click="toExamMsg(item)">{{item.examCourse}}</h4> -->
          <p class="name">{{ item.name }}</p>
          <div class="info">
            <i class="el-icon-loading" /><span>{{ item.examDate }}</span>
